...
description: Use at Domain Discovery when intent includes non-trivial domain behavior, policies, workflows, state transitions, roles, permissions, payments, approvals, quotas, lifecycle, or cross-role collaboration; creates or updates long-lived project or bounded-context domain model artifacts, not one file per requirement.
---
...
Maintain the project's domain model so it can shape user experience, BDD/E2E, technical design, implementation, verification, release learning, and product metrics. Domain Discovery is persistent project knowledge, not a per-requirement document factory.
...
- If a prior run created per-intent domain files, `workflow-migration` should split durable domain knowledge into `.ai/domain/project.md` or `.ai/domain/<bounded-context>.md` and per-intent deltas into `.ai/domain-impact/<intent-id>.md`.

## Handoff

Handoff to Project Foundation Architecture when foundation is required; otherwise to Experience Prototype for user-facing work or Behavior Spec for non-UI behavior.
