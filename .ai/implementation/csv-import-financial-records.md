...
- Added atomic confirm command that creates ordinary `LedgerRecord` rows plus import audit records.
...
- Confirm writes all non-removed valid rows in one transaction.
...
- Confirmation currently supports mapping overrides at submit time; live row revalidation after each mapping change can be improved in a follow-up.
