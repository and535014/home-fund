...
- Create and maintain monthly recurring events for income and expense records.
...
- Prevent duplicate ledger records for the same recurring event occurrence.
...
- Keep recurring event commands and occurrence posting server-validated; UI state cannot be the authority for financial effects.
- Add focused tests for recurring event behavior, duplicate prevention, reminder confirmation, immediate posting, and reporting visibility.
...
- The durable domain model already defines `Recurring event`, `Immediate posting`, `Reminder-based posting`, and `Pending recurring item`.
...
- Existing permissions distinguish admin, finance manager, and general member. The authority to create and manage recurring events still needs a downstream decision.
...
- A `馬上入帳` occurrence creates one and only one ledger record for the scheduled event/month.
...
- Confirming a pending recurring occurrence creates the correct ledger record and preserves trace to the recurring event occurrence.
...
- Domain Discovery / Domain Impact: required, because this introduces recurring event lifecycle, occurrence state transitions, posting policy, duplicate prevention, missed-month handling, and cross-context effects on ledger, reimbursement, and reporting.
...
- Experience Prototype: required, because users need event setup, posting-mode selection, pending reminder visibility, confirmation, and clear distinction between pending and ledgered items.
...
- Who can create or delete recurring events: admin only, finance manager, or both?
- Should a general member be allowed to manage recurring events that only affect themselves?
...
- Should recurring events have pause/end-date behavior in this slice?
...
- Should deleting a recurring event affect pending reminders, or only future not-yet-generated occurrences?
...
- reviewer_focus:
  ...
  - Confirm downstream Domain Discovery should decide authority, missed occurrences, duplicate prevention, and rule delete/recreate semantics.
- must_check:
