...
| Record query | User-selected search, sort, and filter criteria applied to active ledger records in a reporting view. | Reporting |
| Keyword search | A record query criterion that matches approved visible record fields such as record name, note, category name, member display names, and visible type/payment labels. | Reporting |
...
| Batch reimbursement | A batch record action that marks selected active refundable member-paid expenses as reimbursed. User-facing copy may say `批次退款`, but it remains app settlement state. | Reimbursement |
...
| Record query requested | Search and filters apply only to active, readable household records and must not grant mutation permission. Keyword search covers approved visible record fields; category filters use active categories only; date ranges use occurrence dates and allow start-only, end-only, or bounded ranges; reimbursement status filters apply to reimbursement-relevant records. | Search, sort, or filter records | The behavior spec must decide the exact treatment for non-reimbursement-applicable records when reimbursement status is selected. |
...
| LedgerRecord | Income recorded, Expense recorded, Member-paid expense became refundable, Ledger record corrected, Ledger record voided, Batch ledger records voided | Records have amount, month/date, category, creator, payment source, payer/source member, reimbursement status when member-paid, and active/voided lifecycle; general members can modify only owned records; voided records must not appear in active totals, record lists, category summaries, or refundable expense calculations; batch deletion is a multi-record invocation of the same voiding invariant. | Should batch delete fail all selected records when one is ineligible, or void eligible records and report skipped records? |
...
| ReimbursementBatch | Reimbursement expenses selected, Expenses reimbursed, Record detail reimbursement confirmed, Record detail expense reimbursed, Batch reimbursement confirmed, Batch expenses reimbursed | Only finance managers perform reimbursement; selected refundable expenses can be marked reimbursed once; reimbursement totals trace to expense IDs; record-detail reimbursement is the single-expense variant and search batch reimbursement is the multi-expense variant of the same settlement invariant. | Does reimbursement reduce fund balance or only mark settlement state? Should batch refund fail all selected records when one is ineligible, or reimburse eligible records and report skipped records? |
| MonthlyReport | Monthly records viewed, Record query applied, Search records selected, Monthly report generated, Monthly reimbursement table generated, Reimbursement page retired | Reports derive from ledger, recurring, category, and reimbursement data by month; record queries narrow active report records without mutating ledger state; search result selection is temporary and only becomes a mutation through authorized Fund Ledger or Reimbursement commands. | Which mobile report summaries are required for MVP? How should reimbursement-status filters present non-reimbursement-applicable records? |
...
| Reimbursement | reimbursement table, refundable expense, selected expense, reimbursed expense, payer member, record detail reimbursement, batch reimbursement | Calculate refundable member-paid expenses by month and mark selected expenses reimbursed once, including a single-record path from the record detail dialog and a multi-record path from search results. | Uses Fund Ledger expenses and Identity and Access finance-manager permissions; feeds Reporting. |
| Reporting | monthly records, monthly report, record query, keyword search, active category filter, member/fund participation filter, reimbursement status filter, open-ended date range, record sort order, search result selection, category summary, category color, reimbursement status | Month-based views for records, category totals, pending recurring items, and reimbursement status; category summaries use persisted category visual identity when available; record queries narrow active records by approved read-model criteria without changing financial truth; search selection prepares batch commands but does not authorize them. | Downstream read model from Ledger, Categorization, Recurring Schedule, and Reimbursement. |
| Responsive Web Experience | desktop layout, mobile layout, browse flow, create flow, report flow, reimbursement flow, batch action flow, default 404 direct visit | Ensure core workflows are usable on desktop and mobile; refund work is no longer a standalone page and direct removed-route visits use the framework default 404 behavior. | Presentation concern downstream from all domain contexts; not source of financial truth. |
