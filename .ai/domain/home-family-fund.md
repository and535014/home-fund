...
| CSV import batch | A user-confirmed bulk import attempt for one import target, containing parsed rows, validation outcomes, actor, household, and final created records or rejected rows. | Fund Ledger / Reimbursement |
| CSV import target | The approved destination shape for an import file, initially either ledger records or reimbursement payments. | Fund Ledger / Reimbursement / Responsive Web Experience |
...
| Imported reimbursement payment | Reimbursement payment evidence created from a confirmed CSV import batch and linked to the approved reimbursed expense set without creating an ordinary ledger expense. | Reimbursement |
...
| 40 | CSV import file uploaded | Upload CSV import file | Admin or Finance manager | A file is parsed into a target-specific import preview without changing financial records. |
| 41 | CSV import rows validated | Validate CSV import rows | System | Parsed rows are checked against household members, categories, ledger/reimbursement rules, duplicate/conflict policy, and actor permissions. |
| 42 | CSV import confirmed | Confirm CSV import batch | Admin or Finance manager | The actor confirms the reviewed valid rows should become financial records or reimbursement payment evidence. |
| 43 | Ledger records imported | Import ledger records from CSV | Admin or Finance manager | Valid income and expense rows become active ledger records and feed monthly reports under normal ledger rules. |
| 44 | Reimbursement payments imported | Import reimbursement payments from CSV | Finance manager | Valid reimbursement payment rows are linked to approved member-paid expenses without creating ordinary expense records. |
...
| CSV import rows are validated | Each row must match one approved import target, one household, existing members/categories/records, valid dates and amounts, and the actor's authority for the target command. | Validate CSV import rows | Ambiguous member/category/record matches, cross-household references, malformed values, and unresolved duplicate/conflict cases are rejected or blocked for review. |
...
| Reimbursement payment CSV import is confirmed | Imported reimbursement payment evidence must link to the approved member-paid expense set, match the paid-to member and amount policy, and avoid double-counting as ordinary expense. | Import reimbursement payments from CSV | The import must not create payment evidence for voided, fund-paid, income, already-conflicting, cross-member, cross-household, or unauthorized rows. |
...
| ReimbursementBatch | Reimbursement expenses selected, Expenses reimbursed, Record detail reimbursement confirmed, Record detail expense reimbursed, Batch reimbursement confirmed, Batch expenses reimbursed, Reimbursement payment details captured, Reimbursement payment recorded, Reimbursement payments imported | Only finance managers perform reimbursement; selected refundable expenses can be marked reimbursed once; reimbursement totals trace to expense IDs; record-detail reimbursement is the single-expense variant and search batch reimbursement is the multi-expense variant of the same settlement invariant; MVP payment-flow capture records one payment evidence item for one paid-to member and does not create an ordinary expense; reimbursement payment import must preserve the same one-payment-to-one-member and no-double-count invariants unless a later gate changes them. | Should batch refund fail all selected records when one is ineligible, or reimburse eligible records and report skipped records? When should split payments, partial reimbursement, correction, or reversal be introduced? How should imported payment evidence link to underlying expenses? |
...
- CSV import is introduced as a controlled bulk command path, not a separate source of financial truth. Open decisions remain for CSV column contracts, member/category/record matching, duplicate detection, all-or-nothing versus partial-success commit semantics, import history, and whether one import can create both member-paid expenses and reimbursement payment evidence.
