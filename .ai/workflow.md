...
- current_stage: Experience Prototype review for record search, sort, and filter.
- recommended_resume_gate: Behavior Spec / BDD / E2E for `record-search-sort-filter` after explicit prototype approval.
- recommended_next_skill: behavior-spec after prototype approval; experience-prototype if this prototype needs revision; target-aware-release for stricter release target; artifact-prune only if explicitly requested.
