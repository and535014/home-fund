...
- current_stage: Category visual identity and ordering TDD Implementation is drafted and awaiting review.
- recommended_resume_gate: Verification for category visual identity and ordering after implementation approval; optional manual Artifact Prune only if explicitly requested.
- recommended_next_skill: verification for the active change after approval; artifact-prune only if explicitly requested; target-aware-release if a stricter release target is selected.
