...
- current_stage: Target-Aware Release review for record search, sort, and filter local_dev readiness.
- recommended_resume_gate: Learning Loop for `record-search-sort-filter` after explicit Target-Aware Release approval.
- recommended_next_skill: learning-loop after Target-Aware Release approval; target-aware-release if this readiness report needs revision; artifact-prune only if explicitly requested.
