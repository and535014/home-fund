...
- current_stage: Feature Technical Design review for record search, sort, and filter.
- recommended_resume_gate: TDD Implementation for `record-search-sort-filter` after explicit Feature Technical Design approval.
- recommended_next_skill: tdd-implementation after Feature Technical Design approval; feature-technical-design if this design needs revision; target-aware-release for stricter release target; artifact-prune only if explicitly requested.
