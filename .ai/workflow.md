...
- current_stage: Category visual identity and ordering Feature Technical Design is drafted and awaiting review.
- recommended_resume_gate: TDD Implementation for category visual identity and ordering after technical-design approval; optional manual Artifact Prune only if explicitly requested.
- recommended_next_skill: tdd-implementation for the active change after approval; artifact-prune only if explicitly requested; target-aware-release if a stricter release target is selected.
