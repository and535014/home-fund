...
- current_stage: Category visual identity and ordering Target-Aware Release is drafted and awaiting review.
- recommended_resume_gate: Learning Loop for category visual identity and ordering after release-readiness approval; optional manual Artifact Prune only if explicitly requested.
- recommended_next_skill: learning-loop after approval, unless the local_dev-only learning step is explicitly skipped; artifact-prune only if explicitly requested.
