...
- current_stage: Behavior Spec / BDD / E2E review for record search, sort, and filter.
- recommended_resume_gate: Feature Technical Design for `record-search-sort-filter` after explicit Behavior Spec approval.
- recommended_next_skill: feature-technical-design after Behavior Spec approval; behavior-spec if this spec needs revision; target-aware-release for stricter release target; artifact-prune only if explicitly requested.
