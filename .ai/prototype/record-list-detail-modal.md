...
- component_paths:
  ...
  - `src/app/record-category-label.tsx`
  - `src/app/record-list-detail.tsx`
- stack: Next.js App Router, React client component for dialog state, Tailwind, local shadcn-style `Card`, `Dialog`, and `Item` components, lucide icons.
...
- Each item is a full-row `Item` button using the reviewed information arrangement, not the wireframe's border-heavy visual style:
  ...
  - name and payer appear in the first `ItemContent`
...
- Activating an item opens a modal with:
  ...
  - date, category, status, and payer fields
...
- Payer labels show only the actor name: member-paid and income records show the member display name, fund-paid records show `基金`, and unknown members fall back to `成員`.
- No mock API, schema change, route change, or write behavior is introduced.

## States Covered

- normal state: list of recent income/expense records
- income item: green absolute amount and payer/member label when available
...
- The modal uses existing Radix/shadcn-style `Dialog` primitives with title, built-in close button, escape/outside close behavior, and focus management. It intentionally has no `DialogDescription` and no footer action row.
...
- Record item cells follow the reviewed layout: category, name, absolute amount, payer, and date.
- Authenticated user activates a record item and sees a detail modal for that exact record.
- Detail modal shows amount, date, category, status, payer label, and note or empty-note copy.
...
- Payer labels rely on existing member ids in the read model; unknown ids fall back to generic Traditional Chinese labels.
